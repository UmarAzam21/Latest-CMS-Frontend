...
const TITLES: Record<string, string> = { all: "Total Cards Balance", income: KHATA_LABELS.income.title, expense: KHATA_LABELS.expense.title, debt: KHATA_LABELS.debt.title };
                    ...
                    <div className="flex flex-col gap-1.5">
                        {filtered.slice(0, 20).map((e) => (
                            <div key={e.id} className="flex items-center justify-between rounded-brand-8 border border-border-clr px-3 py-2">
                                ...
                                <span className={`para-small font-semibold ${e.kind === "income" ? "text-success" : "text-text-dark"}`}>
                                    {e.kind === "income" ? "+" : "-"}PKR {e.amount.toLocaleString("en-PK")}
                                </span>
                            </div>
                        ))}
                        {filtered.length === 0 && (
                            <p className="para-small text-text-secondary-muted">No entries for this category yet.</p>
                        )}
                    </div>
                </Dialog.Content>
            </Dialog.Portal>
