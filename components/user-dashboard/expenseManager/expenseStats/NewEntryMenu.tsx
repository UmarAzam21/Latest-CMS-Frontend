        ...
        <>
            <DropdownMenu.Root>
                <DropdownMenu.Trigger asChild>
                    <button className="flex items-center gap-1.5 rounded-brand-8 bg-primary px-3 py-2 para-small font-semibold text-white hover:opacity-90">
                        <Plus size={15} /> New Entry <ChevronDown size={13} />
                    </button>
                </DropdownMenu.Trigger>
                <DropdownMenu.Portal>
                    <DropdownMenu.Content align="end" className="z-dropdown w-44 rounded-brand-8 border border-border-clr bg-white p-1 shadow-card-hover">
                        {(["income", "expense", "debt"] as const).map((k) => (
                            <DropdownMenu.Item key={k} onClick={() => setPickedKind(k)} className="cursor-pointer rounded-brand-8 px-2.5 py-2 para-small text-text-secondary hover:bg-page-bg outline-none">
                                {KHATA_LABELS[k].verb}
                            </DropdownMenu.Item>
                        ))}
