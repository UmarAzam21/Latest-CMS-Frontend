...
export default function AdvancedExpenseWorkspaceV2({ entries, categories, cards, onSaved }: AdvancedExpenseWorkspaceProps) {
    ...
    return (
        <section className="mt-5 rounded-brand-16 border border-border-clr bg-page-bg p-3 shadow-card sm:p-5">
            <div className="flex flex-col gap-3 border-b border-border-clr pb-3 sm:flex-row sm:items-center sm:justify-between">
                <div><h2 className="para-small font-semibold text-text-dark">Personal Finance</h2><p className="para-tiny text-text-secondary-muter">{range === "month" ? monthLabel : "All-time activity"}</p></div>
                <NewExpenseDialog categories={categories} cards={cards} onSaved={onSaved} />
            </div>
            <div className="mt-4 grid grid-cols-1 gap-3 xl:grid-cols-[minmax(0,7fr)_minmax(0,3.4fr)]">
                <NetWorthCard chartData={chartData} income={income} expenses={expenses} balance={income - expenses} range={range} onRangeChange={setRange} monthLabel={monthLabel} />
