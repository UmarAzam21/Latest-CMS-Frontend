...
export default function Form({ serviceName, onSubmitted }: RegistrationFormProps) {
	...
	return (
		<form
			...
			className="overflow-hidden rounded-brand-12 border border-border-clr bg-white shadow-card"
		>
			<div className="border-b border-primary/10 bg-gradient-primary-rl px-5 py-5 text-white sm:px-6">
				<div className="flex items-start justify-between gap-4">
					<div>
						<p className="para-tiny font-semibold uppercase tracking-[0.18em] text-white/75">
							Service request
						</p>
						<h2 className="mt-1 text-xl font-bold">{serviceName}</h2>
						<p className="mt-1 para-tiny text-white/80">
			...
			<div className="space-y-6 p-5 sm:p-6">
				<section>
					<div className="mb-3 flex items-center gap-2">
						<span className="flex h-6 w-6 items-center justify-center rounded-full bg-primary-lighter para-tiny font-bold text-primary">
							1
						</span>
					...
					<div className="grid gap-4 sm:grid-cols-2">
						<label>
							<span className="para-small font-medium text-text-dark">Username</span>
							<input
								required
								name="username"
								type="text"
						...
						<label>
							<span className="para-small font-medium text-text-dark">Email address</span>
							<input
								required
								name="email"
								type="email"
						...
						<label>
							<span className="para-small font-medium text-text-dark">Password</span>
						...
						<label>
							<span className="para-small font-medium text-text-dark">CNIC number</span>
							<input
								required
								name="cnic"
								type="text"
				...
				<section className="border-t border-border-clr pt-5">
					<div className="mb-3 flex items-center gap-2">
						...
						<div>
							<h3 className="para-small font-bold text-text-dark">Required documents</h3>
							<p className="para-tiny text-text-secondary-muter">
								Upload clear images or PDF files.
							</p>
						</div>
					</div>
					...
					<div className="grid gap-3 sm:grid-cols-3">
						...
						<FileUpload
							...
							accept="image/*,.pdf"
							fileName={files.certificateFront}
							onChange={handleFileChange}
						/>
						<FileUpload
							...
							accept="image/*,.pdf"
							fileName={files.certificateBack}
							onChange={handleFileChange}
						/>
					</div>
				</section>
				...
				<div className="flex flex-col-reverse items-stretch justify-between gap-3 border-t border-border-clr pt-5 sm:flex-row sm:items-center">
					{submitted ? (
						<p className="flex items-center gap-1.5 para-small font-medium text-green-600">
							<Check size={16} />
					...
					<button
						type="submit"
						className="inline-flex items-center justify-center gap-2 rounded-brand-8 bg-primary px-5 py-3 para-small font-semibold text-white shadow-primary-btn default-transition hover:-translate-y-0.5 hover:opacity-95 focus:outline-none focus:ring-4 focus:ring-primary/20"
					>
						Submit request <UploadCloud size={16} />
					</button>
				</div>
			</div>
		</form>
...
function FileUpload({ name, label, accept, fileName, onChange }: FileUploadProps) {
	return (
		<label className="group flex cursor-pointer flex-col gap-2 rounded-brand-8 border border-dashed border-border-clr bg-page-bg p-3 transition hover:border-primary hover:bg-primary-lighter/30">
			...
			<span className="truncate para-tiny text-text-secondary-muter">
